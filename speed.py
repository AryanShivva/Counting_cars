            ...
            if obj_id in vh_down:
                if cy2 - offset < cy < cy2 + offset:
                    ...
                    if obj_id not in counter:
                        ...
                        info_text = f"ID:{obj_id} Dir:Down Spd:{int(speed_kmh)}km/h Time:{timestamp}"
                        cv2.putText(frame, info_text, (x3, y3 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
            ...
            if obj_id in vh_up:
                if cy1 - offset < cy < cy1 + offset:
                    ...
                    if obj_id not in counter1:
                        ...
                        info_text = f"ID:{obj_id} Dir:Up Spd:{int(speed_kmh1)}km/h Time:{timestamp}"
                        cv2.putText(frame, info_text, (x3, y3 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)
